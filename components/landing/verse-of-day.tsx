...
import { Sparkles, ArrowLeft, ArrowRight, RotateCw, Share2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { VerseShareModal, VerseShareData } from "./verse-share-modal";
import { cn } from "cn";
...
export function VerseOfDay({ dailyVerseData }: VerseOfDayProps) {
  ...
  const shareVersePayload: VerseShareData = {
    textArabic: verseData.arabic.text,
    textTranslation: verseData.translation?.text,
    surahNameArabic: verseData.arabic.surah?.name,
    surahNameEnglish: verseData.arabic.surah?.englishName,
    surahNumber: verseData.arabic.surah?.number,
    ayahNumberInSurah: verseData.arabic.numberInSurah,
  };
      ...
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-card via-card to-primary/5 border border-primary/20 p-6 sm:p-10 shadow-xs transition-all duration-300">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 border-b border-border/50 pb-4">
          ...
          <div className="flex items-center gap-2 self-end sm:self-auto flex-wrap">
            ...
            {/* Share as Image Button in header */}
            <Button
              variant="outline"
              size="sm"
              onClick={() => setIsShareOpen(true)}
              title={t("shareVerseImage")}
              aria-label={t("shareVerseImage")}
        ...
        >
          ...
          <div dir="rtl" className="text-center py-4 sm:py-6 max-w-4xl mx-auto">
            <p className="font-quran text-xl sm:text-2xl lg:text-3xl leading-loose sm:leading-loose text-foreground select-text">
          ...
          <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
            ...
            <Button
              ...
              size="sm"
              onClick={() => setIsShareOpen(true)}
              className="gap-1.5 text-xs text-muted-foreground hover:text-primary border-primary/20 hover:border-primary/40 rounded-full h-8 px-3 cursor-pointer"
            >
              <Share2 className="size-3.5" />
              <span>{t("shareVerseImage")}</span>
            </Button>
          </div>
        </div>
      </section>
